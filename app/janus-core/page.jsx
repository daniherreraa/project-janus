...
function Dropdown({ label, value = '', onChange, options, compact = false, multiple = false }) {
  const [open, setOpen] = useState(false);
  const spanRef = useRef(null);
  const gsapRef = useRef(null);
  const [display, setDisplay] = useState(value ? `[${value}]` : "[########]");
  const [selectedItems, setSelectedItems] = useState(value ? value.split(',').map(item => item.trim()) : []);
  ...
  useEffect(() => {
    if (multiple) {
      if (value) {
        ...
        const newDisplay = items.length > 0 ? `[${items.join(', ')}]` : '[########]';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const ScrambleTextPlugin = pluginModule.ScrambleTextPlugin || pluginModule.default;
  ...
  return (
    <div className="relative">
      <div className={`flex items-center ${compact ? "gap-1" : "gap-1.5 sm:gap-2"}`}>
        ...
        <button
          ...
          className={`group inline-flex items-center ${
            ...
            compact ? "border-transparent bg-transparent" : "border border-white/30 bg-white/5 hover:border-white/50"
        ...
        >
          <span
            ref={spanRef}
            className={`tracking-wide sm:tracking-widest ${value ? "text-[#00C8FF] font-bold" : "text-[#00C8FF] font-bold"}`}
          >
            {display}
          </span>
          <svg className={`${compact ? "h-2.5 w-2.5 sm:h-3 sm:w-3" : "h-3 w-3 sm:h-4 sm:w-4"} text-[#00C8FF]`} viewBox="0 0 20 20" fill="currentColor" aria-hidden>
      ...
      {open && (
        ...
        >
          <div className="max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-[#00C8FF]/30 scrollbar-track-transparent hover:scrollbar-thumb-[#00C8FF]/50 scrollbar-thumb-rounded-full">
            {options.map((opt) => {
              const isSelected = multiple ? selectedItems.includes(opt) : value === opt;
              return (
                <div
                  key={opt}
                  className="w-full"
                  ...
                  <div className="flex items-center w-full font-technor font-bold text-left px-2.5 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs hover:bg-white/10 transition-colors cursor-pointer">
                    {multiple && (
                      <div className="mr-2 flex items-center justify-center w-4 h-4 border border-white/30 rounded-sm">
                        {isSelected && (
                          <svg className="w-3 h-3 text-[#00C8FF]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        )}
                      </div>
                    )}
                    <span className={isSelected ? "text-[#00C8FF]" : "text-white/90"}>
                      {opt}
                    </span>
                  </div>
                </div>
...
const Page = () => {
  ...
  // Calculate visible search results after state is defined
  const visibleSearchResults = searchResults?.articles?.important ? 
    (showAllSearchResults ? searchResults.articles.important : searchResults.articles.important.slice(0, 6)) : [];
  ...
  const articles = [
    { 
      ...
      description: "Latest findings in genomic sequencing",
      link: "https://example.com/genomic-research-2023"
    },
    { 
      id: 2, 
      title: "Protein Analysis", 
      description: "Advanced techniques in protein studies",
      link: "https://example.com/protein-analysis-methods"
    },
    { 
      id: 3, 
      title: "Cell Biology", 
      description: "New discoveries in cellular structures",
      link: "https://example.com/cell-biology-breakthroughs"
    },
    { 
      id: 4, 
      title: "Neuroscience", 
      description: "Brain function and neural networks",
      link: "https://example.com/neuroscience-advancements"
    },
    { 
      id: 5, 
      title: "Immunology", 
      description: "Immune system research updates",
      link: "https://example.com/immunology-research-2023"
    },
    { 
      id: 6, 
      title: "Bioinformatics", 
      description: "Data analysis in biology",
      link: "https://example.com/bioinformatics-tools"
    },
    { 
      id: 7, 
      title: "Microbiology", 
      description: "Microorganisms and their impact",
      link: "https://example.com/microbiology-studies"
    },
    { 
      id: 8, 
      title: "Ecology", 
      description: "Environmental studies",
      link: "https://example.com/ecology-research"
    },
    { 
      id: 9, 
      title: "Biochemistry", 
      description: "Chemical processes in organisms",
      link: "https://example.com/biochemistry-innovations"
    }
  ];
  
  const visibleArticles = showAllArticles ? articles : articles.slice(0, 6);
  
  const carouselItems = [
    { id: 1, title: "Genomic Analysis", description: "Recent advances in genomic sequencing and analysis techniques" },
    { id: 2, title: "Proteomics", description: "Exploring the complex world of proteins and their functions" },
    { id: 3, title: "Metabolomics", description: "Studying chemical processes involving metabolites" },
    { id: 4, title: "Imaging", description: "Advanced imaging techniques in biological research" },
    { id: 5, title: "Neuroscience", description: "Understanding the brain and nervous system" },
  ];
  ...
  const handleSearch = async () => {
    ...
    if (minimizedRef.current) {
      tl.fromTo(
        ...
        { duration: 0.80, ease: "power2.out", clipPath: "inset(0 0% 0 0)", opacity: 1 }
      );
      tl.set(minimizedRef.current, { clearProps: "clipPath" });
    }

    const snapshot = { query, organism, projectType };
    const makeTitle = (s) => (s.organism || s.projectType)
      ? `${s.organism || "Human"} research in ${s.projectType || "various fields"}`
    ...
    tl.add(() => {
      ...
      gsap.delayedCall(0, () => {
        ...
        bootTl.fromTo(
          el,
          { opacity: 0, boxShadow: "0 0 6px var(--holo), inset 0 0 4px var(--holo)" },
          { duration: 0.18, opacity: 0.9, boxShadow: "0 0 18px var(--holo), inset 0 0 12px var(--holo)", ease: "power2.out" }
        );
        bootTl.to(el, { opacity: 0.35, duration: 0.06, yoyo: true, repeat: 5, ease: "steps(2)" });
        bootTl.to(el, { opacity: 0.85, duration: 0.12, yoyo: true, repeat: 3, ease: "steps(2)" });
        const H = el.offsetHeight || el.getBoundingClientRect().height || 0;
        const bandH = sweep.offsetHeight || sweep.getBoundingClientRect().height || 0;
        bootTl.fromTo(
          sweep,
          { y: 0, opacity: 1 },
          { duration: 2, y: Math.max(0, H - bandH), opacity: 1, ease: "power2.inOut" }
        );
  ...
  return (
    <div className="font-supreme">
      <section className="relative w-[100svw] h-[100svh] overflow-hidden">
        <PixelImage src={"https://exnfgivtduajcmskhmrl.supabase.co/storage/v1/object/public/janus-ship/janus-core.png"} alt={"janus background"}/>
        ...
        >
          ...
          {minimized && (
            <div
              ...
              style={{
                ...
                boxShadow: "0 0 12px rgba(0, 200, 255, 0.4), inset 0 0 8px rgba(0, 200, 255, 0.25)",
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 rounded-lg sm:rounded-xl">
                <div
                  ...
                  style={{
                    top: 0,
                    height: "10%",
                    background: "linear-gradient(180deg, rgba(0,200,255,0) 0%, rgba(0,200,255,0.45) 50%, rgba(0,200,255,0) 100%)",
                    filter: "blur(3px)",
                  }}
                />
                <div
              ...
              <div className="relative z-[2] h-full w-full text-white overflow-y-auto">
                ...
                <div
                  ref={minimizedRef}
                  className={`pointer-events-auto absolute left-2 sm:left-4 top-2 sm:top-4 z-[60] overflow-visible flex flex-col sm:flex-row items-start sm:items-center gap-2 text-[10px] sm:text-xs ${
                    minimized && booted ? "opacity-100" : "opacity-0 pointer-events-none"
                  }`}
                  style={{ clipPath: minimized ? undefined : "inset(0 100% 0 0)" }}
                >
                  <button
                    onClick={handleSearch}
                    disabled={isDisabled || isLoading}
                    aria-disabled={isDisabled || isLoading}
                    className={`inline-flex items-center justify-center font-bold rounded-md px-4 sm:px-5 py-1.5 text-xs transition-colors duration-200 font-technor
                      ${isDisabled || isLoading ? "bg-white/30 text-white/80 cursor-not-allowed" : "bg-[var(--accent)] text-whiteo opacity-50"}`}
                  >
                    {isLoading ? (
                ...
                {resultsReady && (
                  <div className="absolute inset-x-2 sm:inset-x-4 lg:inset-x-6 top-16 sm:top-20 lg:top-16 bottom-4 z-[40] text-white overflow-y-auto scrollbar-thin scrollbar-thumb-[#00C8FF]/30 scrollbar-track-transparent hover:scrollbar-thumb-[#00C8FF]/50 scrollbar-thumb-rounded-full transition-colors duration-300">
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
                      ...
                      <div className="flex-1 lg:flex-[0.65] space-y-4">
                        ...
                        {searchResults?.articles?.important?.length > 0 && (
                          <div>
                            <h1 className="font-technor font-bold text-base sm:text-xl lg:text-2xl leading-tight break-words">
                              {typedTitle}
                              <span aria-hidden className={`ml-0.5 ${typingTitle ? "inline-block animate-pulse" : "hidden"} text-[var(--accent)]`}>_</span>
                            </h1>
                            <div className="mt-2 max-w-2xl">
                              <div className="flex items-center gap-2 text-xs sm:text-sm text-[var(--text-dim)] cursor-pointer group" onClick={() => setShowFullDesc(!showFullDesc)}>
                                <span className="font-bold font-technor">Description</span>
                                <svg 
                                  className={`w-3 h-3 transition-transform duration-200 ${showFullDesc ? 'rotate-90' : ''}`} 
                                  fill="none" 
                                  viewBox="0 0 24 24" 
                                  stroke="currentColor"
                                >
                              ...
                              {showFullDesc && (
                                <div className="mt-1">
                                  <p className="text-xs sm:text-sm text-[var(--text-dim)] font-supreme mb-3">
                                    {typedDesc}
                                    <span aria-hidden className={`ml-0.5 ${typingDesc ? "inline-block animate-pulse" : "hidden"} text-[var(--accent)]`}>_</span>
                                  </p>
                                  {searchResults?.generated?.description && (
                                    <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-3">
                                      <p className="text-xs text-white/80 font-supreme">
                                        {searchResults.generated.description}
                                      </p>
                                    </div>
                                  )}
                        ...
                        <div className="relative w-full">
                          <h3 className="text-xs sm:text-sm font-bold tracking-wide mb-3">
                            {searchResults ? 'Search Results' : 'Important Articles'}
                            {searchResults?.counts?.total_studies > 0 && (
                              <span className="ml-2 text-[var(--accent)]">
                                ({searchResults.counts.important} of {searchResults.counts.total_studies} studies)
                              </span>
                            )}
                          </h3>
                          ...
                          {isLoading ? (
                          ...
                          ) : searchResults && searchResults.articles?.important?.length === 0 ? (
                            <div className="bg-white/5 border border-white/20 rounded-lg p-6 text-center">
                              <div className="flex flex-col items-center gap-3">
                                ...
                                <div>
                                  <h4 className="text-sm sm:text-base text-white mb-1 font-technor font-semibold">We couldn't find any result for this search</h4>
                                  <p className="text-xs sm:text-sm text-white/70 font-supreme">
                                    Try to adjust the filters or use diferent keywords
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            <>
                              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                {visibleSearchResults.map((article, index) => (
                                  <div key={`${article.id}-${index}`} className="h-full group">
                                    <a 
                                    ...
                                    >
                                      <div className="relative rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-1.5 sm:p-2 h-full transition-all duration-300 hover:bg-white/10 hover:border-white/30">
                                        ...
                                        <div className="px-0.5">
                                          <div className="text-[10px] sm:text-xs font-bold leading-tight mb-0.5 group-hover:text-[var(--accent)] transition-colors line-clamp-2">
                                            {article.title}
                                          </div>
                                          <div className="text-[9px] sm:text-[10px] text-white/60 group-hover:text-white/80 line-clamp-1">
                                            {article.project_type || 'Space Research'}
                                          </div>
                                        </div>
                                      </div>
                                    </a>
                              ...
                              {searchResults.articles.important.length > 6 && (
                                <div className="mt-3 text-center">
                                  ...
                                  >
                                    {showAllSearchResults ? 'Show Less' : `Show More (${searchResults.articles.important.length - 6})`}
                                  </button>
                                </div>
                              )}
                            ...
                            <>
                              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                {visibleArticles.map((article) => (
                                  <div key={article.id} className="h-full group">
                                    <a 
                                      href={article.link} 
                                      target="_blank" 
                                      rel="noopener noreferrer"
                                    ...
                                    >
                                      <div className="relative rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-1.5 sm:p-2 h-full transition-all duration-300 hover:bg-white/10 hover:border-white/30">
                                        <div className="aspect-[16/9] rounded-md bg-white/20 border border-white/25 mb-1.5 overflow-hidden">
                                          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-white/10 to-white/5">
                                            <span className="text-white/40 text-[10px]">Preview</span>
                                          </div>
                                        </div>
                                        <div className="px-0.5">
                              ...
                              {articles.length > 6 && (
                                <div className="mt-3 text-center">
                                  <button 
                                    ...
                                    className="text-[10px] sm:text-xs font-medium text-[var(--accent)] hover:text-white transition-colors px-3 py-1.5 rounded-md border border-[var(--accent)]/30 hover:bg-[var(--accent)]/10"
                                  >
                                    {showAllArticles ? 'Show Less' : `Show More (${articles.length - 6})`}
                                  </button>
                      ...
                      {searchResults?.articles?.important?.length > 0 && (
                        <div className="flex-1 lg:flex-[0.4] flex items-center justify-center mt-6 lg:mt-0">
                          <div className="relative w-full max-w-md lg:max-w-lg">
                            <div className="relative" style={{ height: 'clamp(280px, 50vw, 400px)' }}>
                              {/* Navigation Arrows - On sides of the center item */}
                              <button 
                                className="absolute -left-2 sm:left-2 md:left-4 top-1/2 transform -translate-y-1/2 z-20 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
                              ...
                              {/* Responsive circular path */}
                              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[200px] sm:w-[250px] lg:w-[320px] h-[200px] sm:h-[250px] lg:h-[320px] rounded-full">
                                {carouselItems.map((item, index) => {
                                  const position = (index - currentCarouselIndex + carouselItems.length) % carouselItems.length;
                                  const angle = (position * (2 * Math.PI)) / carouselItems.length - Math.PI / 2;
                                  const radius = typeof window !== 'undefined' && window.innerWidth < 640 ? 80 : window.innerWidth < 1024 ? 100 : 120;
                                  ...
                                  return (
                                    <div 
                                      key={index}
                                      className="absolute w-28 h-36 sm:w-32 sm:h-40 lg:w-36 lg:h-44 bg-white/5 rounded-lg border border-white/20 backdrop-blur-sm transition-all duration-300 ease-in-out cursor-pointer group hover:bg-white/10 hover:border-white/30 hover:shadow-lg hover:shadow-[var(--accent)]/20"
                                      style={{
                                        left: `calc(50% + ${x}px)`,
                                        top: `calc(50% + ${y}px)`,
                                    ...
                                    >
                                      <div className="w-full h-20 sm:h-22 lg:h-24 bg-white/10 group-hover:bg-white/20 rounded-t-lg flex items-center justify-center transition-colors">
                                        <span className="text-white/60 group-hover:text-white/80 text-xs sm:text-sm transition-colors">Image</span>
                                      </div>
                                      <div className="p-2 sm:p-3">
                                        <div className="text-xs sm:text-sm font-medium mb-1 line-clamp-1 group-hover:text-[var(--accent)] transition-colors">
                                          {item.title}
                                        </div>
                                        <div className="text-[10px] sm:text-xs text-white/70 group-hover:text-white/90 line-clamp-2 transition-colors">
                                          {item.description}
                                        </div>
                                      </div>
                                    </div>
                              ...
                              <button 
                                className="absolute -right-2 sm:-right-2 md:right-4 top-1/2 transform -translate-y-1/2 z-20 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
          ...
          <div
            ...
            style={{ clipPath: minimized ? "inset(0 100% 0 0)" : undefined, opacity: minimized ? 0 : 1, pointerEvents: minimized ? "none" : "auto" }}
          >
            {showHolo && (
              <div
                ref={holoRef}
            ...
            <div className="w-full flex flex-wrap justify-center items-center gap-2 sm:gap-3 md:gap-4 bg-transparent font-technor">
              <button
                ...
                disabled={isDisabled || isLoading}
                aria-disabled={isDisabled || isLoading}
                className={`inline-flex items-center justify-center rounded-md px-8 sm:px-10 md:px-12 py-2 font-technor font-bold text-xs sm:text-sm transition-colors duration-200
                  ${isDisabled || isLoading ? "bg-white/30 text-white/80 cursor-not-allowed" : "bg-[var(--accent)] text-white hover:brightness-110 active:brightness-95 opacity-50"}`}
              >
                {isLoading ? (
                  <>

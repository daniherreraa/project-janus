...
const Page = () => {
  ...
  const visibleSearchResults = searchResults?.articles?.important ? 
    (showAllSearchResults ? searchResults.articles.important : searchResults.articles.important.slice(0, 3)) : [];
                    ...
                    <div className="flex justify-evenly flex-col lg:flex-row gap-4 lg:gap-6">
                          ...
                          {isLoading ? (
                            ...
                            <div className="bg-white/5 border border-white/20 rounded-lg p-6 text-center">
                              <div className="flex flex-col items-center gap-3">
                                ...
                                <div>
                                  <h4 className="text-sm sm:text-base font-bold text-white mb-1 font-technor">No results found</h4>
                                  <p className="text-xs sm:text-sm text-white/70 font-technor">
                                    Try adjusting the filters or using different keywords
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            <>
                              <div className="grid grid-cols-1 gap-3 sm:gap-4">
                                {visibleSearchResults.map((article, index) => {
                                  ...
                                  const fullStudy = searchResults?.data?.studies_full?.find(
                                    study => study['Study Identifier'] === article.id
                                  );
                                          ...
                                          <div className="flex flex-col sm:flex-row gap-3">
                                            ...
                                            <div className="flex-1">
                                              ...
                                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-xs text-white/60 mb-2 font-technor">
                                                <div>
                                                  <span className="font-medium text-white/70">ID:</span> {article.id}
                                                </div>
                                                <div>
                                                  <span className="font-medium text-white/70">Score:</span> {(article.rank_score * 100).toFixed(1)}%
                                                </div>
                                                <div>
                                                  <span className="font-medium text-white/70">Date:</span> {new Date(article.release_date).toLocaleDateString()}
                                                </div>
                                                {article.DOI && (
                                                  <div className="col-span-2 sm:col-span-3 truncate">
                                                    <span className="font-medium text-white/70">DOI:</span> {article.DOI}
                                                  </div>
                                                )}
                                              </div>
                                              
                                              {/* Description */}
                                              {fullStudy?.['Study Description'] && (
                              ...
                              {searchResults.articles.important.length > 3 && (
                                <div className="mt-3 text-center font-technor">
                                  ...
                                  >
                                    {showAllSearchResults ? 'Show Less' : `Show More (${searchResults.articles.important.length - 3})`}
                                  </button>
                                </div>
                              )}
                            ...
                            <>
                              ...
                              {articles.length > 3 && (
                                <div className="mt-3 text-center">
                                  ...
                                  >
                                    {showAllArticles ? 'Show Less' : `Show More (${articles.length - 3})`}
                                  </button>
                      ...
                      {searchResults?.articles?.important?.length > 0 && (
                        <div className="w-full lg:w-[35%] space-y-4 font-technor">
                          ...
                          <div className="rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="bg-white/5 rounded-md p-3 border border-white/10">
                                ...
                                <div className="text-xs text-white/60 mt-1">Total Studies</div>
                              </div>
                              <div className="bg-white/5 rounded-md p-3 border border-white/10">
                                <div className="text-2xl font-bold text-[var(--accent)]">
                                  {searchResults.counts.important}
                                </div>
                                <div className="text-xs text-white/60 mt-1">Important</div>
                              </div>
                            </div>
                          </div>

                          {/* Emerging Topics */}
                          {searchResults?.topics?.emerging && searchResults.topics.emerging.length > 0 && (
                            <div className="rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-4">
                              <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                                <svg className="w-4 h-4 text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <div className="space-y-2">
                                {searchResults.topics.emerging.slice(0, 5).map((topic, idx) => (
                                  <div key={idx} className="flex items-center justify-between bg-white/5 rounded-md p-2 border border-white/10 hover:bg-white/10 transition-colors">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div className="w-12 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                        <div 
                                          ...
                                          style={{ width: `${(topic.subset_occurrences / Math.max(...searchResults.topics.emerging.map(t => t.subset_occurrences))) * 100}%` }}
                                        />
                                      </div>
                                    </div>
                                  </div>
                          ...
                          {/* Frequent Terms */}
                          {searchResults?.topics?.frequent_subset && searchResults.topics.frequent_subset.length > 0 && (
                            <div className="rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-4">
                              <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                                <svg className="w-4 h-4 text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {searchResults.topics.frequent_subset.slice(0, 8).map((term, idx) => (
                                  <div 
                                    key={idx}
                                    className="px-2.5 py-1 bg-white/5 rounded-full border border-white/10 hover:bg-white/10 transition-colors"
                                  >
                                    <span className="text-xs text-white font-medium">{term.token}</span>
                                    <span className="text-xs text-[var(--accent)] ml-1.5">×{term.occurrences}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {(organism || projectType || query) && (
                            <div className="rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-4">
                              ...
                              <div className="space-y-2">
                                ...
                                {organism && (
                                  <div className="text-xs">
                                    <span className="text-white/60">Organism:</span>
                                    <span className="ml-2 text-white font-medium">{organism}</span>
                                  </div>
                                )}
                                {projectType && (
                                  <div className="text-xs">
                                    <span className="text-white/60">Project Type:</span>
                                    <span className="ml-2 text-white font-medium">{projectType}</span>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          {searchResults?.data?.studies_full && (
                            <div className="rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm p-4">
                              ...
                              <div className="space-y-2">
                                {(() => {
                                  const projectCounts = {};
                                  searchResults.data.studies_full.forEach(study => {
                                    const type = study.project_label || 'Unknown';
                                    projectCounts[type] = (projectCounts[type] || 0) + 1;
                                  });
                                  const total = Object.values(projectCounts).reduce((a, b) => a + b, 0);
                                  return Object.entries(projectCounts).map(([type, count]) => (
            ...
            <div className="w-full flex flex-wrap justify-center items-center gap-2 sm:gap-3 md:gap-4 bg-transparent font-technor">
              <button
                ...
                className={`inline-flex items-center justify-center rounded-md px-8 sm:px-10 md:px-12 py-2 font-bold text-xs sm:text-sm transition-colors duration-200
                  ${isDisabled || isLoading ? "bg-white/30 text-white/80 cursor-not-allowed" : "bg-white/5 hover:bg-white/10 border-white/10 text-[#00C8FF] hover:brightness-110 active:brightness-95 border-[1px] border-[var(--accent)]"}`}
              >
                {isLoading ? (
                  <>
